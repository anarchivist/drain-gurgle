      ...
      if(results > 0){
        ...
        for (var facet_value in concat_fields){
          ...
          for (var i = 0; i<sorted_list.length; i++){
            ...
            ${sorted_list[i][0]} (${sorted_list[i][1]})</a></li>`
            if (i == view_facets + 1){
        ...
        for (var querytype in pairs){
          ...
          for (var query = 0; query < queries.length; query++) {
            ...
            if (querytype.indexOf('facet') > -1){
              ...
              $(`#${facet_id}`).append(' <img src="/assets/vendor/bootstrap-icons/x.svg" alt="Remove facet" title="Remove facet" role="img">');
            }
            breadcrumbs += `<button class="btn btn-outline-secondary btn-sm" type="button" onclick="remove_facet('${querytype}=${encode_facet}')">
                ${querytype.replace("facet_", "").charAt(0).toUpperCase()}${querytype.replace("facet_", "").slice(1,)}: ${facet_data} <img src="/assets/vendor/bootstrap-icons/x.svg" alt="Remove facet" title="Remove facet" role="img">
                </button>`
          }
        }
        breadcrumbs += search_items.length > 1 ? `<button class="btn btn-outline-secondary btn-sm" type="button" onclick="remove_facet('all')">
            Clear All <img src="/assets/vendor/bootstrap-icons/x.svg"  role="img">
            </button>` : ''
        breadcrumbs += `</span>`
